...
def create_route_segment(segment: RouteSegmentCreate, db: Session = Depends(get_db), current_user=Depends(get_current_company_user)):
...
def list_route_segments(route_id: str = None, db: Session = Depends(get_db), current_user=Depends(get_current_company_user)):
    ...
    segments = query.all()
    # Manual population if not using relationship eager loading effectively in this context,
    # but normally Pydantic from_attributes works if the relationship is lazy loaded.
    # However, let's ensure we return valid Pydantic models with names populated from relationships.
    response = []
    for s in segments:
        resp = RouteSegmentResponse.model_validate(s)
        resp.start_station_name = s.start_station.name if s.start_station else "Unknown"
        resp.end_station_name = s.end_station.name if s.end_station else "Unknown"
        response.append(resp)
    return response

# Get a single segment
@router.get("/{segment_id}", response_model=RouteSegmentResponse)
def get_route_segment(segment_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_company_user)):
...
def update_route_segment(segment_id: str, segment_update: RouteSegmentUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_company_user)):
    segment = db.query(RouteSegment).filter(RouteSegment.id == segment_id, RouteSegment.company_id == current_user.company_id).first()
    if not segment:
        raise HTTPException(404, "Route segment not found")
...
def delete_route_segment(segment_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_company_user)):
    segment = db.query(RouteSegment).filter(RouteSegment.id == segment_id, RouteSegment.company_id == current_user.company_id).first()
    if not segment:
        raise HTTPException(404, "Route segment not found")
